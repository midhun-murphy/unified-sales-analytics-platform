...
base_path = f's3://sportsbar-data-s3/{data_source}'
...
# Fix order_qty data type to match existing bronze table schema
if spark.catalog.tableExists(bronze_table):
    # Get the existing table's schema for order_qty
    table_schema = spark.table(bronze_table).schema
    order_qty_field = [field for field in table_schema.fields if field.name == "order_qty"]
    
    if order_qty_field:
        target_type = order_qty_field[0].dataType.simpleString()
        print(f"Bronze table order_qty type: {target_type}")
        print(f"Current DataFrame order_qty type: {df.schema['order_qty'].dataType.simpleString()}")
        
        # Cast order_qty to match the table's data type
        df = df.withColumn("order_qty", F.col("order_qty").cast(target_type))
        print(f"Casted order_qty to: {target_type}")
else:
    print("Bronze table does not exist yet - no casting needed for first load")
